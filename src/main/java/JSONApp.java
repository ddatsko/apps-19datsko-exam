...
public class JSONApp {
    ...
    private static JsonObject makeCourse(String name, int mark, boolean passed) {
        return new JsonObject(new JsonPair("course", new JsonString(name)),
    ...
    public static JsonObject sessionResult() {
        JsonObject main = (new BasicStudent("Andrii", "Rodionov", 2)).toJsonObject();
        JsonObject oop = makeCourse("OOP", 3, true);
        JsonObject english = makeCourse("English", 5, true);
        JsonObject math = makeCourse("Math", 2, false);

        main.add(new JsonPair("exams", new JsonArray(oop, english, math)));
